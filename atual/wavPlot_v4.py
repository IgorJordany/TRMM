...
con = psycopg2.connect(host='localhost', database='trmm', user='postgres', password='123')

# Lendo os dados
arquivo = [] # Lista
cur = con.cursor()
cur.execute("SELECT tratado FROM anual where cod_ponto = 1999 and data_anual = '2007-01-01'") # Select no banco
rows = cur.fetchall()

for row in rows:
	sst = np.array(row[0],dtype=float) # Recuperando string do banco para numpyarray float
print(sst)
...
n = len(sst)
dt = 0.0223
time = np.arange(len(sst)) * dt + 1996.0  # construct time array
xlim = ([1998,1998])  # plotting range
