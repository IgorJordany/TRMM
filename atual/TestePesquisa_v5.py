...
def busca():
	...
	for i in rang(83073):
		...
		for row1 in rows1: # Obtendo dados do banco de dados e convertendo em formato utilizavel
			...
			codPontoB = np.array(row1[2],dtype=float)
			distBruto = np.array(row1[3],dtype=float)
			distWavelet = np.array(row1[4],dtype=str)
		aux=(distWavelet,codPontoB)
		lista.append(aux)
...
for i in range(1,2):
	...
	for j in range(i,83105):
		...
		if Acod_ponto != Bcod_ponto:
			...
			cur.execute("")
			cur.execute("INSERT INTO aux (cod_pontoa, cod_pontob, distanciabruto, distanciatratado, dataa, datab) VALUES ('%s','%s','{%s}','{%s}','%s','%s')" %(Acod_ponto, Bcod_ponto, distBruto, distWavelet, Adata, Bdata))
	busca()
