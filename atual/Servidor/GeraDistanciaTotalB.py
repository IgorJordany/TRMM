...
# Conectando no banco
log=open('dist_total_inicioB.txt','w')
data = datetime.now()
...
for i in range(1,2598):
	...
	for j in range(1,2598):
		...
		if Acod_ponto != Bcod_ponto:
			...
			aux=(distBruto, distWavelet, Bcod_ponto)
			lista.append(aux)
	lista.sort()
	lista6primeiros=[]
...
log=open('dist_total_terminoB.txt','w')
data = datetime.now()
log.write("data: "+str(data.day)+" / "+str(data.month)+" / "+str(data.year)+"\n"+"tempo: "+str(data.hour)+":"+str(data.minute)+":"+str(data.second))
log.close()