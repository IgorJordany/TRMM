...
cur.execute("SELECT bruto, tratado FROM total") # Select no banco
SB = cur.fetchall()

print("Carregou Select")
log=open('Gera_Matriz_Total_Inicio.txt','w')
data = datetime.now()
log.write("data: "+str(data.day)+" / "+str(data.month)+" / "+str(data.year)+"\n"+"tempo: "+str(data.hour)+":"+str(data.minute)+":"+str(data.second))
log.close()
Bruto=[]
Wavelet=[]
MatB=np.zeros((2597,2597))
MatW=np.zeros((2597,2597))
for i in SB:
	Bruto.append(i[0])
	Wavelet.append(i[1])

for PA in range(0,2597):
	for PB in range(PA,2597):
		...
		MatB[PA][PB] = DistB
		MatW[PA][PB] = DistW
print("Calculo distancia pronto")
...
log=open('Gera_Matriz_Total_Termino.txt','w')
data = datetime.now()
log.write("data: "+str(data.day)+" / "+str(data.month)+" / "+str(data.year)+"\n"+"tempo: "+str(data.hour)+":"+str(data.minute)+":"+str(data.second))
log.close()
