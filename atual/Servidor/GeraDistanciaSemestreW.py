...
# Conectando no banco
log=open('dist_semestre_inicioW.txt','w')
data = datetime.now()
...
log=open('dist_semestre_terminoW.txt','w')
data = datetime.now()
log.write("data: "+str(data.day)+" / "+str(data.month)+" / "+str(data.year)+"\n"+"tempo: "+str(data.hour)+":"+str(data.minute)+":"+str(data.second))
log.close()