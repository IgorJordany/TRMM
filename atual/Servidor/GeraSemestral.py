	...
	for j in range(16):
		...
		for k in range(2):
			...
			if k == 0:	
				...
				print('Ano: 01/%s ate 07/%s' %(ano,ano))
			else:
				print("Id ponto: %s" %(ide))
				print('Ano: 07/%s ate 01/%s' %(ano,ano+1))
			arquivo = [] # Lista
			if k == 0:
				cur.execute("SELECT precipitac FROM trmm where lat = %s and lon = %s and datatrmm >= '%s/01/01' and datatrmm <= '%s/07/01' order by datatrmm limit 180" %(lat,lon,ano,ano)) # Select no banco
			elif k == 1:
				cur.execute("SELECT precipitac FROM trmm where lat = %s and lon = %s and datatrmm >= '%s/07/01' and datatrmm <= '%s/01/01' order by datatrmm limit 180" %(lat,lon,ano,ano+1)) # Select no banco
			rows = cur.fetchall()
			for row in rows:
			...
			if k == 0:
				cur.execute("INSERT INTO semestral (cod_ponto, bruto, tratado, data_anual) VALUES ('%s', '{%s}', '{%s}', '%s/01/01')" %(ide, bruto, tratado, ano))
			else:	
				cur.execute("INSERT INTO semestral (cod_ponto, bruto, tratado, data_anual) VALUES ('%s', '{%s}', '{%s}', '%s/07/01')" %(ide, bruto, tratado, ano))
