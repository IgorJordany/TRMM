...
cur = con.cursor()
# Lendo os dados
for i in range(1,41553):
	cur.execute("SELECT * FROM anual where id = %s" %(i)) # Select no banco
	rows1 = cur.fetchall()
	...
	for j in range(i,41553):
		cur.execute("SELECT * FROM anual where id = %s" %(j)) # Select no banco
		rows2 = cur.fetchall()
		...
		if Acod_ponto != Bcod_ponto:
			...
			cur.execute("INSERT INTO distanciaanual (cod_pontoa, cod_pontob, distanciabruto, distanciatratado, dataa, datab) VALUES ('%s','%s','{%s}','{%s}','%s','%s')" %(Acod_ponto, Bcod_ponto, distBruto, distWavelet, Adata, Bdata))
